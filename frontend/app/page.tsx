...
export default function Home() {
  ...
  const markManual=async(job:NormalizedJob)=>{if(!window.confirm("Confirm that you personally submitted this application on the employer's website?"))return;setPackageBusy(true);setPackageNotice(null);try{const response=await apiFetch(`${api}/applications/jobs/${job.id}/manually-applied`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({confirmed:true,confirmation:"Confirmed manually applied by authenticated user"})});const body=await response.json().catch(()=>null) as (ExistingApplication&{message?:string})|null;if(!response.ok)throw new Error(body?.message||`Manual status update failed (${response.status})`);if(body)setApplications(values=>[...values.filter(item=>item.jobId!==job.id),body]);setJobs(values=>values.map(value=>value.id===job.id?{...value,status:"MANUALLY_APPLIED"}:value));setSelected(null);setActive("Manually Applied");setPackageNotice("Application recorded in Manually Applied.");}catch(error){setPackageNotice(error instanceof Error?error.message:"Manual status update failed");}finally{setPackageBusy(false);}};
    ...
    <main>
      ...
      {active === "Resume Versions" ? <ResumeVersions apiBase={api}/> : active === "Settings" ? <Settings apiBase={api} apiFetch={apiFetch}/> : active === "Add Job Manually" ? <ManualJobForm apiBase={api} apiFetch={apiFetch} onCreated={value=>{const job=normalizeJob(value as Job,0);setJobs(items=>[job,...items]);setActive("New Jobs");void selectJob(job);}}/> : workflowViews.includes(active) ? <ApplicationWorkflow apiBase={api} view={active}/> : <>
        ...
        <section className="panel">
          ...
          {loading ? <div className="state">Loading opportunities…</div> : shown.length === 0 ? <div className="state">No jobs match these filters.</div> : <div className="tableWrap"><table><thead><tr><th>ROLE</th><th>LOCATION</th><th>MATCH</th><th>STATUS</th><th>POSTED</th><th>ACTION</th></tr></thead><tbody>{shown.map(job => <tr key={job.id} onClick={() => void selectJob(job)}><td><b>{job.title}</b><small>{job.companyName||"Company unavailable"} · {(job.sourcePortal||job.source).replaceAll("_", " ")}</small></td><td>{job.location}</td><td><span className="score">{jobScores[job.id]?`${Math.round(jobScores[job.id].score)}%`:"Review"}</span></td><td><span className={`badge ${job.status.toLowerCase()}`}>{job.status.replaceAll("_", " ")}</span></td><td>{formatDate(job.postedAt)}</td><td><button className="tableAction" disabled={packageBusy||job.id.startsWith("demo")||job.status==="MANUALLY_APPLIED"} onClick={event=>{event.stopPropagation();void markManual(job);}}>{job.status==="MANUALLY_APPLIED"?"Applied":"Mark manually applied"}</button></td></tr>)}</tbody></table></div>}
