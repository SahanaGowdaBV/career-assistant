...
const nav = ["Overview", "New Jobs", "Add Job Manually", "High Score Jobs", "Pending Review", "Ready to Apply", "Failed Applications", "Manually Applied", "Resume Versions", "Cover Letters", "Companies", "Analytics", "Settings"];
const navIcons = ["⌂", "＋", "✎", "★", "◷", "→", "!", "✓", "▤", "✉", "◇", "↗", "⚙"];
...
export default function Home() {
  ...
  const [manualBusyJobId,setManualBusyJobId]=useState<string|null>(null);
  const [packageNotice,setPackageNotice]=useState<string|null>(null);

  const fetchDashboardData=useCallback(async()=>{
    const [jobsResponse,applicationsResponse]=await Promise.all([
      apiFetch(`${api}/jobs/page?size=50`,{cache:"no-store"}),
      apiFetch(`${api}/applications`,{cache:"no-store"}),
    ]);
    if(!jobsResponse.ok)throw new Error(`Jobs refresh failed (${jobsResponse.status})`);
    const data=await jobsResponse.json() as JobPage;
    return {
      jobs:Array.isArray(data.content)?data.content.map(normalizeJob):demo,
      applications:applicationsResponse.ok?await applicationsResponse.json() as ExistingApplication[]:null,
    };
  },[apiFetch]);

  useEffect(() => {
    fetchDashboardData()
      .then(data=>{setJobs(data.jobs);if(data.applications)setApplications(data.applications);setOffline(false);})
      .catch(error => {if (!(error instanceof AuthenticationRequiredError)) setOffline(true);})
      .finally(() => setLoading(false));
  }, [fetchDashboardData]);

  const filtered = useMemo(() => {
    const sectionStatuses = jobStatusesForView[active];
    return jobs
      .filter(job => active !== "Overview" || isActiveOpportunityStatus(job.status))
      .filter(job => !sectionStatuses || sectionStatuses.includes(job.status))
      .filter(job => (job.title + " " + job.location).toLowerCase().includes(query.toLowerCase()) && (status === "ALL" || job.status === status))
      .sort((a, b) => sort === "title" ? a.title.localeCompare(b.title) : dateValue(b.postedAt) - dateValue(a.postedAt));
  }, [active, jobs, query, status, sort]);
  const shown = filtered.slice((page - 1) * 8, page * 8);
  const counts = {New:jobs.filter(job => job.status === "NEW").length, "High score":jobs.filter(job => job.status === "HIGH_SCORE").length, "Pending review":jobs.filter(job => job.status === "PENDING_REVIEW").length, "Manually applied":jobs.filter(job => job.status === "MANUALLY_APPLIED").length};

  const navigate = (name:string) => {setActive(name); setSelected(null); setStatus("ALL"); setPage(1);};
  const workflowViews=["Pending Review","Ready to Apply","Auto Applied","Failed Applications","Manually Applied","Cover Letters"];
  ...
  const markManual=async(job:NormalizedJob)=>{if(!window.confirm("Confirm that you personally submitted this application on the employer's website?"))return;setManualBusyJobId(job.id);setPackageNotice(null);try{const response=await apiFetch(`${api}/applications/jobs/${job.id}/manually-applied`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({confirmed:true,confirmation:"Confirmed manually applied by authenticated user"})});const body=await response.json().catch(()=>null) as (ExistingApplication&{message?:string})|null;if(!response.ok)throw new Error(body?.message||`Manual status update failed (${response.status})`);if(body)setApplications(values=>[...values.filter(item=>item.jobId!==job.id),body]);setJobs(values=>values.map(value=>value.id===job.id?{...value,status:"MANUALLY_APPLIED"}:value));setSelected(null);const refreshed=await fetchDashboardData();setJobs(refreshed.jobs);if(refreshed.applications)setApplications(refreshed.applications);setOffline(false);setPackageNotice("Application recorded in Manually Applied. Jobs refreshed.");}catch(error){setPackageNotice(error instanceof Error?error.message:"Manual status update failed");}finally{setManualBusyJobId(null);}};
    ...
    <main>
      ...
      {active === "Resume Versions" ? <ResumeVersions apiBase={api}/> : active === "Settings" ? <Settings apiBase={api} apiFetch={apiFetch}/> : active === "Companies" ? <Companies apiBase={api} apiFetch={apiFetch}/> : active === "Add Job Manually" ? <ManualJobForm apiBase={api} apiFetch={apiFetch} onCreated={value=>{const job=normalizeJob(value as Job,0);setJobs(items=>[job,...items]);setActive("New Jobs");void selectJob(job);}}/> : workflowViews.includes(active) ? <ApplicationWorkflow apiBase={api} view={active}/> : <>
        ...
        <section className="panel">
          ...
          {loading ? <div className="state">Loading opportunities…</div> : shown.length === 0 ? <div className="state">No jobs match these filters.</div> : <div className="tableWrap"><table><thead><tr><th>ROLE</th><th>LOCATION</th><th>MATCH</th><th>STATUS</th><th>POSTED</th><th>ACTION</th></tr></thead><tbody>{shown.map(job => <tr key={job.id} onClick={() => void selectJob(job)}><td><b>{job.title}</b><small>{job.companyName||"Company unavailable"} · {(job.sourcePortal||job.source).replaceAll("_", " ")}</small></td><td>{job.location}</td><td><span className="score">{jobScores[job.id]?`${Math.round(jobScores[job.id].score)}%`:"Review"}</span></td><td><span className={`badge ${job.status.toLowerCase()}`}>{job.status.replaceAll("_", " ")}</span></td><td>{formatDate(job.postedAt)}</td><td><button className="tableAction" disabled={manualBusyJobId===job.id||job.id.startsWith("demo")||job.status==="MANUALLY_APPLIED"} onClick={event=>{event.stopPropagation();void markManual(job);}}>{manualBusyJobId===job.id?"Saving…":job.status==="MANUALLY_APPLIED"?"Applied":"Mark manually applied"}</button></td></tr>)}</tbody></table></div>}
