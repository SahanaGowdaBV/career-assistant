...
Before launch, confirm Railway health is green, Vercel production variables are present, Supabase redirect/storage settings are correct, and no secret file is tracked. Do not run migrations or real cleanup without a reviewed production change window.
